...
The JAGS model is compiled using the `compile_JAGS` function. We just use 3 herds to test the code. We declare two risk factors of new infection: `ln_nOrig6_12` and `LocalSeroPrev`. This will result in 3 coefficient associated with the log-odds probability of new infection: intercept, coefficient for `ln_nOrig6_12` and coefficient for `LocalSeroPrev`.
...
The samples from the model parameter posterior distributions are stored in the `parameters` component of the variable in which we have stored the model results. We store them in a variable called param to explore these distributions.
...
The samples from the model predicted probability of infection posterior distributions are stored in the `proba_inf` component of the variable in which we have stored the model results. We store them in a variable called `proba_inf` to explore these distributions.
