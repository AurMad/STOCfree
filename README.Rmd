...
For running the model the data needs to be put in a special format: the STOCfree_data class. The aim is to help setting the different parameters required and to help checking the different hypotheses that are made. The creation of a STOCfree_data is realised using the `STOCfree_data()` function.

## Test and risk factor data

In order to demonstrate how to include categorical data in the model, we create a categorical variable called from `purch_yn` (for purchase yes/no) from the `ln_nOrig6_12` variable.
