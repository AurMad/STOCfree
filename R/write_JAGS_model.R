...
###   Status = discrete
#####################################################################################
write_JAGS_model.herd_discrete <- function(data){
...
#' @export
write_JAGS_model.herd_rf_discrete <- function(data){
...
#' @export
write_JAGS_model.animal_discrete <- function(data){
...
write_JAGS_model.animal_rf_discrete <- function(data){
...
#######  Status = proba
###################################################################################
write_JAGS_model.herd_proba <- function(data){
...
##############################################################################
  ...
  ## test result
  p_test_pos_typ1[i1] <- Se[test_id_typ1[i1]] * pi[status_typ1[i1]] +
                          (1 - Sp[test_id_typ1[i1]]) * (1 - pi[status_typ1[i1]])
  ...
  # probability of infection given previous status and dynamics
  pi[status_typ2[i2]] <- tau1 * (1 - pi[status_typ2[i2] - 1]) +
                         tau2 * pi[status_typ2[i2] - 1]

  ## status sampled from probability
  # Status[status_typ2[i2]] ~ dbern(pi[status_typ2[i2]])

  ## test result
  p_test_pos_typ2[i2] <- Se[test_id_typ2[i2]] * pi[status_typ2[i2]] +
                          (1 - Sp[test_id_typ2[i2]]) * (1 - pi[status_typ2[i2]])
  ...
  ## test result
  p_test_pos_typ3[i3] <- Se[test_id_typ3[i3]] * pi[status_typ3[i3]] +
                          (1 - Sp[test_id_typ3[i3]]) * (1 - pi[status_typ3[i3]])
...
'},
  ...
  # probability of infection given previous status and dynamics
  pi[status_no_test[j]] <- tau1 * (1 - pi[status_no_test[j] - 1]) +
                           tau2 * pi[status_no_test[j] - 1]
...
for(i4 in 1:n_status_typ4){
  ...
  # probability of infection given previous status and dynamics
  predicted_proba[herd_id_pr4[i4]] <- tau1 * (1 - pi[status_typ4[i4] - 1]) +
                 tau2 * pi[status_typ4[i4] - 1]
...
for(i5 in 1:n_status_typ5){
  ...
  # probability of infection given previous status and dynamics
  pi5_init[i5] <- tau1 * (1 - pi[status_typ5[i5] - 1]) +
                  tau2 * pi[status_typ5[i5] - 1]
...
#' @export
write_JAGS_model.herd_rf_proba <- function(data){
...
##############################################################################
  ...
  # probability of infection given previous status and dynamics
  pi[status_typ2[i2]] <- tau1[status_typ2[i2]] * (1 - pi[status_typ2[i2] - 1]) +
                         tau2 * pi[status_typ2[i2] - 1]
  ...
  # probability of infection given previous status and dynamics
  pi[status_no_test[j]] <- tau1[status_no_test[j]] * (1 - pi[status_no_test[j] - 1]) +
                           tau2 * pi[status_no_test[j] - 1]
...
for(i4 in 1:n_status_typ4){
  ...
  # probability of infection given previous status and dynamics
  predicted_proba[herd_id_pr4[i4]] <- tau1[status_typ4[i4]] * (1 - pi[status_typ4[i4] - 1]) +
                 tau2 * pi[status_typ4[i4] - 1]
...
for(i5 in 1:n_status_typ5){
  ...
  # probability of infection given previous status and dynamics
  pi5_init[i5] <- tau1[status_typ5[i5]] * (1 - pi[status_typ5[i5] - 1]) + tau2 * pi[status_typ5[i5] - 1]
...
#' @export
write_JAGS_model.animal_proba <- function(data){
...
##############################################################################
  ...
  ## test result
  p_test_pos_typ1[i1] <- Se[test_id_typ1[i1]] * pi[status_typ1[i1]] * pi_within +
                          (1 - Sp[test_id_typ1[i1]]) * (1 - pi[status_typ1[i1]] * pi_within)
  ...
  ## test result
  p_test_pos_typ2[i2] <- Se[test_id_typ2[i2]] * pi[status_typ2[i2]] * pi_within +
                          (1 - Sp[test_id_typ2[i2]]) * (1 - pi[status_typ2[i2]] * pi_within)
...
write_JAGS_model.animal_rf_proba <- function(data){
...
###   Status = predict
#####################################################################################
write_JAGS_model.herd_predict <- function(data){
...
##############################################################################
  ...
  # probability of infection given previous status and dynamics
  pi2_init[i2] <- tau1 * (1 - pi[status_typ2[i2] - 1]) +
                  tau2 * pi[status_typ2[i2] - 1]
...
#' @export
write_JAGS_model.herd_rf_predict <- function(data){
...
##############################################################################
  ...
  # probability of infection given previous status and dynamics
  pi2_init[i2] <- tau1[status_typ2[i2]] * (1 - pi[status_typ2[i2] - 1]) +
                  tau2 * pi[status_typ2[i2] - 1]
...
#' @export
write_JAGS_model.animal_predict <- function(data){
...
write_JAGS_model.animal_rf_predict <- function(data){
