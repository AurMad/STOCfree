...
cat('model{
  ...
  for(j in 1:n_pred_test){
    ...
    piPred[ind_last_is_test[j]] <- test_res[ind_p[ind_last_is_test[j]]] *
    ...
    StatusPred[ind_last_is_test[j]] ~ dbern(piPred[ind_last_is_test[j]])
  ...
  for(k in 1:n_pred_no_test){
    ...
    piPred[ind_last_is_not_test[k]] <- tau1[ind_p[ind_last_is_not_test[k]]] * (1 - Status[ind_f[ind_last_is_not_test[k]]]) +
      tau2 * Status[ind_f[ind_last_is_not_test[k]]]

    StatusPred[ind_last_is_not_test[k]] ~ dbern(piPred[ind_last_is_not_test[k]])
  ...
  for(i_rf in 1:n_risk_factors){
    ...
    theta[i_rf] ~ dnorm(theta_norm_mean, theta_norm_prec)
