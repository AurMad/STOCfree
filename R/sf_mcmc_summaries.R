...
#' @param sfd the STOC free data object used to run the model that generate the MCMC samples
...
herd_summary <- function(x = data.frame(),
  ...
  x_type_check <- unlist(sapply(c(".chain", ".iteration", ".draw", "herd_id", "predicted_proba"), grep, colnames(x)))
  if(length(x_type_check) < 5) stop("Wrong type of input")

  unique_herds <- unique(x$herd_id)

  ## checks that the same herds are present in x and sfd
  herd_corresp <- sfd$herd_id_corresp

  if(length(na.omit(match(unique_herds, herd_corresp$herd_id))) < length(unique_herds)) stop("The numbers of herds differ between x and sfd")
  ...
  hrd_smmry <- merge(herd_corresp,
                     hrd_smmry,
                     by = "herd_id")

  hrd_smmry <-  hrd_smmry[, -match("herd_id", colnames(hrd_smmry))]
